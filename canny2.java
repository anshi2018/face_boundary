...
public class canny2{
	...
	static float lowThreshold ;
        ...
        static float lowMultiplier=0.13f;
	static float highMultiplier=0.151f;
	...
	static int smoothingOrder=1;
	static float smoothingSigma=5f;
	...
	public static BufferedImage gaussianSmoothing(BufferedImage img) throws IOException {
		...
		File f = new File("F:\\Programs\\smoohed_" + base + ".jpg");
	...
	public static void follow(int i, int j) {
		...
		for (int k = -1; k <= 1; k++) {
			for (int l = -1; l <= 1; l++) {
				if (checkBounds(i+k, j+l, order)&& mag[i + k][j + l] > lowThreshold && mag[i + k][j + l] < highThreshold) {
	...
	public static float autoThresholdMedian(BufferedImage image) throws IOException {
       ...
       static int j;
        public static void constructIntermediate(BufferedImage img,float[][] magValue,boolean flag) throws IOException{
           
            WritableRaster raster = intermediate.getRaster();
	    int[] pixels = new int[intermediate.getHeight()];
	    for (int y = 0; y < intermediate.getWidth(); y++) {
	        raster.getPixels(0, y, intermediate.getHeight(), 1, pixels);
	        for (int i = 0; i < pixels.length; i++) {
	              pixels[i]=Math.abs((int)magValue[y][i]);
           ...
           /* for(int i=0;i<img.getWidth();i++){
                ...
                {
                    img.setRGB(i, j, Math.abs((int)magValue[i][j]));
                    
                }
            }*/
            if(flag)
            {
            File f = new File("F:\\Programs\\intermediate_x_" + base +num+ ".jpg");
			if (!ImageIO.write(img, "JPEG", f)) {
				throw new RuntimeException("Unexpected error writing image");
			}
            }
            ...
            {
                  File f = new File("F:\\Programs\\intermediate_y_" + base +num+ ".jpg");
			if (!ImageIO.write(img, "JPEG", f)) {
				throw new RuntimeException("Unexpected error writing image");
			}
            }
	...
	public static void main(String[] args) throws IOException {
		...
		CascadeClassifier faceDetector = new CascadeClassifier(("F:\\Programs\\haarcascade_frontalface_alt.xml"));
		Mat image = Highgui.imread(("F:\\Programs\\" + base + ".jpg"));
		BufferedImage src = ImageIO.read(new File("F:\\Programs\\" + base + ".jpg"));
		...
		for (Rect rect : faceDetections.toArray()) {
			if (rect.y - (rect.height / 6)*2 >= 0 && rect.y + rect.height + (rect.height / 6)*2 <= src.getHeight()  &&
				...
				dest = src.getSubimage(rect.x-rect.width/6, rect.y - (rect.height / 6)*2, rect.width+rect.width / 3, rect.height + 2*rect.height / 3);
			}
                        else if  (rect.y - rect.height / 6 >= 0 && rect.y + rect.height + rect.height / 6 <= src.getHeight() &&
                                rect.x - (rect.width / 6) >= 0 &&rect.x + rect.width + (rect.width / 6) <= src.getWidth()) {
				// System.out.println(flag);
				dest = src.getSubimage(rect.x-rect.width/6, rect.y - rect.height / 6,  rect.width+rect.width / 3, rect.height + rect.height / 3);
			} 
			...
			File f = new File("F:\\Programs\\corp_" + base + num + ".jpg");
			...
			convol(xMag, overImg, xKernel);
                       // constructIntermediate(intermediate, xMag,true);
                        
			convol(yMag, overImg, yKernel);
                       // constructIntermediate(intermediate, yMag,false);
			...
			// writing
			f = new File("F:\\Programs\\zzz_" + base +num+ ".jpg");
